...
class Datagrid extends Table
{
    ...
    public function createDataGrid($obj_db, $str_db_table = '', $int_num2show = 30, $bool_editable = false)
    {
        ...
        // sql string
        $_sql_str = 'SELECT ' . $this->sql_column .
        ...
        $_num_query = $obj_db->query($_sql_str_count);
        ...
        while ($_data = $this->grid_real_q->fetch_row()) {
            ...
            foreach ($this->modified_content as $_field_num => $_new_content) {
                ...
                if (isset($this->grid_result_rows[$_row][$_field_num])) {
                    if (is_callable($_new_content)) {
                        $this->grid_result_rows[$_row][$_field_num] = call_user_func_array($_new_content, [$obj_db, $this->grid_result_rows[$_row], $_field_num]);
                    } else
                        ...
                        if (preg_match('@^callback\{.+\}@i', $_new_content)) {
                            ...
                            if (function_exists($_callback_func)) {
                                ...
                                $this->grid_result_rows[$_row][$_field_num] = $_callback_func;
                            }
                        } else {
            ...
            if ($this->editable) {
                ...
                if ($this->edit_property) {
                    ...
                    $_edit_link = '<a class="editLink' . (!$this->using_AJAX ? ' notAJAX' : '') . '" '
                        . 'href="' . $_SERVER['PHP_SELF'] . '?' . $_edit_data . '&' . $_url_query_str . '" data-post="' . $_edit_data . '" title="Edit">' . $this->edit_link_text . '</a>';
            ...
            $this->setCellAttr($_row, null, 'class="' . $_row_class . '" data-id="'.$_data[0].'"');
    ...
    protected function makeOutput($int_num2show = 30)
    {
        ...
        if ($this->num_rows > $int_num2show && !$this->disable_paging) {
            $_paging = Paging::paging($this->num_rows, $int_num2show, 5);
        }
        // iframe
        $_iframe = '';
        // setting form target
        ...
        if ($this->editable) {
            if (class_exists('simbio_form_maker')) {
                $form_maker = new FormMaker($this->table_name, $this->chbox_form_URL, $str_form_method = 'post', false);
            ...
            $_button_grp = '<div class="d-flex justify-content-between">';
            // if checkbox is include then show button
            if ($this->chbox_property) {
                $_button_grp .= '<div class="btn-group btn-group-sm" role="group" aria-label="Small button group">';
                $_button_grp .= '<input type="button" data-table-name="' . $this->table_name . '" data-message="' . $this->chbox_confirm_msg . '" value="' . ($this->chbox_action_button ? $this->chbox_action_button : __('Delete Selected Data')) . '" class="checkboxFormSubmit s-btn btn ' . ($this->chbox_action_button ? 'btn-outline-success' : 'btn-outline-danger') . '" />';
                $_button_grp .= '<input type="button" value="' . $_check_all . '" class="check-all button btn btn-outline-secondary" />';
                $_button_grp .= '<input type="button" value="' . $_uncheck_all . '" class="uncheck-all button btn btn-outline-secondary" />';
                $_button_grp .= '</div>';
            }

            // paging
            if ($_paging) {
                $_button_grp .= '<div class="paging-area">' . $_paging . "\n" . '</div>';
            }
            $_button_grp .= '</div>' . "\n";
            ...
            $_buffer .= $_button_grp . '<div class="table-responsive my-3">' . $this->printTable() . '</div>' . $_button_grp;
